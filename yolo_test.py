...
path = './testImage'
# path = '../../DishSet/JPEGImages/'  # 待检测图片的位置
# test_path = '../../DishSet/ImageSets/Main/test.txt'
...
class YOLO(object):
    _defaults = {
        "model_path": 'logs/000/ep253-loss16.027-val_loss15.605.h5',
    ...
    def detect_image(self, image):
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            file.write(
                ' ' + str(self.class_names.index(predicted_class) + 1) + ' ' + str(score) + ' ' + str(left) + ' ' + str(
                    top)
...
if __name__ == '__main__':
    ...
    yolo = YOLO()
    # f = open(test_path)
    # testlsits = f.read().splitlines()
    # f.close()
    # print(testlsits)
    for filename in os.listdir(path):
        # if filename.split('.')[0] in testlsits:
